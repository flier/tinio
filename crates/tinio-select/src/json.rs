...
//!
//! `JsonReader` yields one `Record::Json` per reached element. Framing:
...
use crate::row::Value as RowValue;
use crate::record::RecordReader;
use crate::row::Record;
use crate::sql::{FromClause, PathSeg};
...
/// AWS input-record cap (same contract as `record.rs`): a record over
/// 1 MB errors the stream out. LINES: line span (content + terminator);
/// DOCUMENT: whole stream.
...
/// Case-insensitive field lookup in encounter order (traversed paths;
/// the engine's select-side lookup adds its own ambiguity error).
pub(crate) fn field_ci<'a>(map: &'a serde_json::Map<String, Value>, name: &str) -> Option<&'a Value> {
    map.iter()
...
impl<R: Read> JsonReader<R> {
    pub fn new(reader: R, ty: JsonType, from: &FromClause) -> Self {
        Self {
    ...
    fn cap() -> SelectError {
        SelectError::Format(CAP_MESSAGE.into())
    }
    ...
    fn read_line(&mut self) -> Result<Option<(u64, Vec<u8>)>, SelectError> {
    ...
    /// The whole DOCUMENT stream, bounded by the same 1 MB cap.
    fn read_document(&mut self) -> Result<Option<Vec<u8>>, SelectError> {
    ...
    fn expand(&self, value: Value) -> Vec<Option<Value>> {
        let mut cands: Vec<Option<Value>> = match self.ty {
            JsonType::Lines => vec![Some(value)],
            JsonType::Document => match value {
...
fn apply(seg: &PathSeg, cands: Vec<Option<Value>>) -> Vec<Option<Value>> {
    ...
    for cand in cands {
        match seg {
            ...
            PathSeg::Name(name) => match cand {
                None => out.push(None),
                Some(Value::Object(map)) => out.push(field_ci(&map, name).cloned()),
                Some(_) => out.push(None),
            },
...
impl<R: Read> RecordReader for JsonReader<R> {
    fn next(&mut self) -> Result<Option<Record>, SelectError> {
        loop {
            ...
            match self.ty {
                JsonType::Lines => match self.read_line()? {
...
fn parse(bytes: &[u8]) -> Result<Value, SelectError> {
    serde_json::from_slice(bytes)
        .map_err(|e| SelectError::Format(format!("json input: {e}")))
}
...
mod tests {
    ...
    use crate::engine::{Engine, OutRow};
    use crate::record::RecordReader;
    ...
    fn read(sql: &str, ty: JsonType, input: &str) -> Vec<Record> {
        let plan = parse(sql).unwrap_or_else(|e| panic!("{sql}: {e}"));
        let mut r = JsonReader::new(Cursor::new(input.as_bytes().to_vec()), ty, &plan.from);
        let mut out = Vec::new();
    ...
    fn run(sql: &str, ty: JsonType, input: &str) -> Vec<OutRow> {
        let plan = parse(sql).unwrap_or_else(|e| panic!("{sql}: {e}"));
        let mut r = JsonReader::new(Cursor::new(input.as_bytes().to_vec()), ty, &plan.from);
    ...
    fn run_err(sql: &str, ty: JsonType, input: &str) -> crate::SelectError {
        let plan = parse(sql).unwrap_or_else(|e| panic!("{sql}: {e}"));
        let mut r = JsonReader::new(Cursor::new(input.as_bytes().to_vec()), ty, &plan.from);
        let mut engine = Engine::new(plan);
        let mut result = None;
    ...
    fn empty_document_and_array_yield_nothing() {
        assert_eq!(
            read("SELECT * FROM S3Object", JsonType::Document, ""),
            Vec::<Record>::new()
        );
...
"#,
    ...
    fn traversal_zero_match_wildcard_emits_one_missing() {
        ...
        for sql in [
            ...
            assert_eq!(
                read(sql, JsonType::Lines, "{\"other\": 1}\n"),
                vec![Record::Json(None)],
                "{sql}"
            );
        }
    ...
    fn traversal_scalar_price_rows() {
        ...
        assert_eq!(recs, vec![Record::Json(Some(json!(10))), Record::Json(Some(json!(20)))]);
    }
    ...
    fn document_object_root_is_one_row_object() {
        ...
        assert_eq!(recs, vec![Record::Json(Some(json!({"a": 1, "b": "x", "c": [1, 2]})))]);
    }
    ...
    fn lines_input_record_cap() {
        ...
        let mut r = JsonReader::new(Cursor::new(input), JsonType::Lines, &plan.from);
        match r.next() {
            Err(crate::SelectError::Format(msg)) => {
                assert_eq!(msg, "input record exceeds 1 MB");
                assert_eq!(
                    crate::SelectError::Format(msg).to_string(),
    ...
    fn document_input_record_cap() {
        ...
        let plan = parse("SELECT * FROM S3Object").unwrap();
        let mut r = JsonReader::new(Cursor::new(doc), JsonType::Document, &plan.from);
        match r.next() {
            Err(crate::SelectError::Format(msg)) => assert_eq!(msg, "input record exceeds 1 MB"),
            other => panic!("expected format error, got {other:?}"),
        }
    }
    ...
    fn document_input_record_cap_exact_allowed() {
        ...
        let recs = read("SELECT * FROM S3Object", JsonType::Document, &doc);
        assert_eq!(recs, vec![Record::Json(Some(json!("a".repeat(1024 * 1024 - 2))))]);
    }
...
"#;
    ...
    fn select_id_extra_columns_match_aws() {
        ...
        let rows = run("SELECT id FROM S3Object[*].Rules[*].id", JsonType::Lines, AWS_EXAMPLE);
    ...
    fn json_missing_projection_is_missing_not_null() {
        ...
        let rows = run("SELECT id FROM S3Object s", JsonType::Lines, "{\"expr\": \"y\"}\n");
        assert_eq!(rows[0].vals, vec![Field::Missing]);
    }

    #[test]
    fn json_null_attribute_is_null_not_missing() {
        let rows = run("SELECT id FROM S3Object s", JsonType::Lines, "{\"id\": null}\n");
...
"#;
        let rows = run("SELECT _1.dir_name, _1.owner FROM S3Object[*]", JsonType::Lines, input);
        assert_eq!(rows.len(), 2);
...
"#;
        let rows = run("SELECT d.dir_name, d.files FROM S3Object[*] d", JsonType::Lines, input);
        assert_eq!(rows[0].keys, vec!["dir_name", "files"]);
        // Nested JSON stays a `Json` value; CSV output rejects it later.
        assert_eq!(
            rows[0].vals,
            vec![s("important_docs"), Field::Present(Value::Json(Box::new(json!([{"name": "."}]))))]
        );
    }

    #[test]
    fn underscore_one_field_wins_over_whole_record() {
        let rows = run("SELECT _1 FROM S3Object s", JsonType::Lines, "{\"_1\": \"x\"}\n");
        assert_eq!(rows[0].vals, vec![s("x")]);
        let rows = run("SELECT _1 FROM S3Object s", JsonType::Lines, "{\"a\": 1}\n");
        assert_eq!(
            rows[0].vals,
    ...
    fn raw_number_passthrough_no_decimal_parse() {
        ...
        let rows = run("SELECT x FROM S3Object s", JsonType::Lines, "{\"x\": 1e309}\n");
        assert_eq!(rows[0].keys, vec!["x"]);
        assert_eq!(rows[0].vals, vec![raw("1e+309")]);
        let rows = run("SELECT * FROM S3Object s", JsonType::Lines, "{\"x\": 1e309}\n");
        assert_eq!(rows[0].vals, vec![raw("1e+309")]);
        // A plain integer token is verbatim.
        let rows = run("SELECT x FROM S3Object s", JsonType::Lines, "{\"x\": 123456789012345678901234567890}\n");
    ...
    fn json_attribute_lookup_case_sensitivity() {
        ...
        let rows = run("SELECT s.name FROM S3Object s", JsonType::Lines, "{\"NAME\": \"a\"}\n");
    ...
    fn json_attribute_case_insensitive_duplicates_are_ambiguous() {
        ...
        match run_err(
            ...
            crate::SelectError::Ambiguous(m) => assert_eq!(m, "name"),
