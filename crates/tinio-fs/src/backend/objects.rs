...
impl FsStorage {
    ...
    async fn resolve_object_info(
        ...
        bucket: &bucket::Name,
        key: &object::Key,
    ) -> Result<(fs::File, u64, SystemTime, ETag), Error> {
    ...
    async fn copy_object_fast(
        ...
        // MD5 of the copied bytes. The stored-ETag read and the copy are
        // independent (the copy is needed regardless of the ETag
        // outcome) — run them concurrently.
        ...
        let unchanged = fs::metadata(&path).await.is_ok_and(|now| {
            now.len() == size
