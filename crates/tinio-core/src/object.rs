...
mod tests {
    ...
    #[test]
    fn object_key_reserved_flag() {
        assert!(key(".tinio/state").unwrap().is_reserved());
        assert!(key("a/b/.tinio/c").unwrap().is_reserved());
        assert!(key(r"a\.tinio\b").unwrap().is_reserved());
        assert!(!key("atinio/file").unwrap().is_reserved());
        assert!(!key(r"a\atinio\b").unwrap().is_reserved());
    }
    ...
    fn absolute_paths_rejected() {
        for key in ["/", "/a", "//a", "a/b/"] {
            if key == "a/b/" {
                validate_object_key(key).unwrap();
                continue;
            }
            assert!(
                validate_object_key(key).is_err(),
    ...
    fn drive_letter_paths_rejected() {
        // Windows drive-letter absolute/relative paths would escape the
        // storage root there; a `:` elsewhere in the key stays valid.
        for key in [r"C:\foo", "C:/foo", "C:foo", r"d:\.tinio\state"] {
            assert!(
                validate_object_key(key).is_err(),
                "{key:?} must be rejected"
            );
        }
        validate_object_key("dir/a:b").unwrap();
        validate_object_key("ab:c").unwrap();
    }

    #[test]
    fn control_characters_rejected() {
        for key in ["a\x00b", "a\x1fb", "a\x7fb", "a\nb", "a\tb"] {
            assert!(
                validate_object_key(key).is_err(),
                "{key:?} must be rejected"
            );
    ...
    #[test]
    fn tinio_reserved_at_any_depth() {
        for k in [
            ".tinio",
            ".tinio/x",
            "a/.tinio",
            ...
            assert!(key(k).unwrap().is_reserved(), "{k:?} must be reserved");
        }
        for k in ["atinio", ".tiniox", "a/atinio/b", "x.tinio", "a/x.tinio/b"] {
            assert!(!key(k).unwrap().is_reserved(), "{k:?} must not be reserved");
        }
    }
