...
- Q: Are symlinks in the storage root followed? → A: **No by default** — access resolving through a symlink is rejected and symlink entries are excluded from listings (secure default: a link inside a bucket cannot escape the storage root); opt-in via `[storage.fs] follow_symlinks = true`.
...
- What happens when an object key resolves through a symlink? Symlinks are **rejected by default** (`follow_symlinks = false`): such requests fail and link entries are excluded from listings. When `follow_symlinks = true` (opt-in — the root is user-owned and links may point outside it, documented), access resolves through links.
