...
- What happens when the user presses Ctrl+C or sends SIGTERM to a foreground server? The server performs the same graceful shutdown as `POST /stop` (cease accepting, bounded (10 s) drain, state file and socket removed, exit 0); a second signal exits immediately without draining. `SIGHUP` is ignored and logged (no config reload in v1). On Windows, Ctrl+C and console-close events (closing the console window, logoff, system shutdown) trigger the same graceful path. (FR-014)
...
- **FR-018**: The server MUST provide a management plane separate from the S3 data plane: token-authenticated, exposed by default over a local unix socket (or Windows named pipe), exposing `GET /status`, `POST /stop` (graceful shutdown with a bounded (10 s) in-flight drain), `GET /metrics`, and `GET /openapi.json`; binding the control channel MUST enforce single-instance semantics. The management API MUST additionally be exposable over TCP HTTP or HTTPS per configuration, in which case ALL endpoints require the token. (Optional in minimal builds; transport and endpoint details are specified in the management API contract.) Graceful shutdown MUST additionally be triggered by SIGINT/SIGTERM on unix and by Ctrl+C / console-close events on Windows, following the same path as `POST /stop` (cease accepting, bounded drain, state/socket cleanup, exit 0); a second signal exits immediately without draining, and SIGHUP is ignored and logged (no config reload in v1).
