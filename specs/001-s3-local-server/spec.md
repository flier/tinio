...
- **FR-024**: The server MUST avoid slow first listings over externally-populated directories: ETag metadata MUST be pre-computed in the background after startup (configurable and rate-limitable, never blocking startup, yielding to request traffic, aborting quietly on shutdown), so externally-added files become cheap to list; listings MUST remain correct when the feature is disabled. (Implementation details in the research document.)
