...
|---|---|---|---|
| `OBJECT_META` | `(bucket: &str, key: &str)` | `(etag_hex: &str, size: u64, mtime_nanos: u64, file_identity: u64, tags_wire: &str, checksum_wire: &str, owner_wire: &str, acl_wire: &str)` | composite key makes per-bucket range scans free (walk, remove_bucket); identity is used for composed-ETag touch/replace discrimination (§5.5.7). The tags and checksum elements are **empty strings when the object has none** (spec 2026-08-31); the checksum wire is `<algorithm wire>:<base64 value>:<kind>` — e.g. `CRC32:NhCmhg==:FULL_OBJECT` — with the kind (`FULL_OBJECT`/`COMPOSITE`) recorded at write time so read paths never derive it. **The owner and ACL elements were extended 2026-09-05 (FR-033)**: `owner_wire` = the canonical ID (`""` = no recorded owner — the auth layer resolves the lazy default owner), `acl_wire` = the canonical grants wire (the codec's `to_grants_wire`; `""`/garbage → the default private ACL with no owner — the codec self-heals, in `tinio-store`'s row decode); the row extracts also decode (`owner`/`acl`), so the read paths never re-parse. The ACL elements self-heal together with the established tags style; garbage elements self-heal to empty/`None` on read (like the etag) |
| `BUCKETS` | `name: &str` | `(created_at_nanos: u64, tags_wire: &str, owner_wire: &str, acl_wire: &str)` | aligns with the existing buckets.json semantics; the tags element is empty when the bucket has none; **owner/ACL elements added 2026-09-05 (FR-033)** — same wire rules as `OBJECT_META` (the 4-tuple: `BUCKETS.value` = created, tags, owner, acl) |
| `UPLOADS` | `(bucket: &str, upload_id: &str)` | `(key: &str, initiated_at_nanos: u64, tags_wire: &str, owner_wire: &str, acl_wire: &str)` | upload existence = record existence (replaces upload.json); the tags element is the create-time object tag set, applied to the object at completion; **owner/ACL elements added 2026-09-05 (FR-033)** — the create-time owner + ACL pair the completion applies internally (the object owner = the CMU creator) — same wire rules as above (the 5-tuple) |
...
| `STATE` | `"version"` | `1` | validated at open: absent → write it (new DB); present but mismatched → error (mirrors buckets.json's version behavior); also holds the `compact_needed` marker row (see §5.8). **Version stays 1 across additive schema changes** — `UPLOAD_CHECKSUMS`/`PART_CHECKSUMS` and the tags/checksum elements + `OBJECT_PARTS` carried no bump (user ruling 2026-09-02: additive changes carry no version; a same-version DB from an older format may fail at row decode — delete and rebuild by hand); the owner/ACL elements (2026-09-05, FR-033) follow the same rule — additive, no bump (user ruling 2026-09-05) |

Encoding conventions: etags are stored as `ETag::as_str()` hex strings, validated into the domain type via `ETag::new` on read — consistent with the existing "storage layer uses plain strings, validation on read" convention; `mtime_nanos` reuses the existing `mtime_nanos()` helper. Tags persist as the canonical wire string (sorted `k=v&k2=v2`, RFC-3986 percent-encoded, shared with the `x-amz-tagging` header codec); read paths parse with the tolerant `tags_from_wire` mirror — the 10-cap core `parse_wire` cannot serve the 50-tag bucket cap. Owner/ACL wires (2026-09-05, FR-033) follow the same plain-string convention: `owner_wire` = the canonical account ID, `acl_wire` = the canonical grants string (`grant1&grant2…`, sorted `grantee_wire,PERMISSION` — the owner element is its OWN row element, never part of the grants wire); read paths decode through the shared tinio-store codec (`decode_owner_wire` / `decode_acl_wire` — domain-invalid → `None` / the default private ACL, the established self-heal style).
...
`Error` gains a `Database(database::Error)` variant (aligned with tinio-mem); the five redb sub-variants (`Open`/`Transaction`/`Table`/`Storage`/`Commit`) are defined **once** in tinio-store (`tinio_store::error::Error` — **thiserror-derived `#[from]` per variant**, display strings verbatim) and are nested by fs `database::Error` through a transparent `Redb` variant (`#[from]`) and aliased by mem (`DatabaseError`, same derive) — 2026-08-25 revision: the draft's "explicit `From` per variant, no derived `#[from]`" contradicted the implementation; derive is authoritative, docs/style.md synced; 2026-09-03 revision (shared-store plan): the `derive_more` `#[from(forward)]` suggestion was rejected (E0119 coherence conflict) — the mechanism is thiserror `#[from]` on the wrapping variants plus explicit constructor hops at the raw-redb sites (`Error::Redb(e.into())`/`Error::Database(e.into())`), no hand-written `From` impls — plus fs's own lifecycle sub-variants `Compaction(#[from] redb::CompactionError)`/`Io(#[from] io::Error)`; struct variants `UnsupportedVersion { path, found, expected }` and `CorruptMeta { key, source }` (explicit constructors `database::error::unsupported_version`/`corrupt_meta`); redb failures project to the contract `storage::Error::Io`. Note: the spec review draft once listed five top-level variants, of which `Storage(StorageError)` collided with the existing contract passthrough variant `Error::Storage(storage::Error)`; settled by nesting per style.md "redb nests in `Database(database::Error)`". **Existing-variant disposition (grilling G4 decision + 2026-08-25 revision)**: `Json` + `CorruptStateFile` removed (JSON-specific, disappear with the old layout; `InvalidMetaEntry` does not actually exist in the current error.rs, so there is nothing to remove); integrity is now `check_integrity`'s job, corrupt state is expressed as `Database(database::Error::Redb(tinio_store::error::Error::Open(..)))`; **no top-level `UnsupportedStateVersion`** — a STATE-table version mismatch is the nested `database::Error::UnsupportedVersion`, lifted to `Error::Database(..)` by the crate-level `From` (`database::Error::Io` is unwrapped separately to `Error::Io`); `InvalidPath`/`RootNotDirectory` stay.
...
| G4 | error variant disposition | **decided (grilling)**: remove `Json`/`InvalidMetaEntry`/`CorruptStateFile` (integrity is `check_integrity`'s job; corrupt state expressed as `Database(database::Error::Redb(tinio_store::error::Error::Open(..)))`); `InvalidPath`/`RootNotDirectory` stay. **Revised (2026-08-25)**: no top-level `UnsupportedStateVersion` — STATE version-check failure is the nested `database::Error::UnsupportedVersion`, lifted by the crate-level `From` to `Error::Database(..)`; redb/io sub-variants use thiserror-derived `#[from]` (the original "explicit From, no derive" contradicted the implementation; docs/style.md synced) |
