...
- [X] T137 [US1] Specs, docs & cucumber: FR-034 (SelectObjectContent) written into `contracts/s3-surface.md` (bullet + automated-coverage row), checklist item CHK052 in `checklists/compatibility.md`, `select.feature` + step wiring per `docs/tests.md` (CSV filter by column position, count(*) over JSON lines, LIMIT, gzip-compressed CSV), task entry here — FR-034
...
### Addendum 2026-09-05 — owners & ACLs (design `docs/superpowers/specs/2026-09-05-s3-acl-owner-design.md`, plan `docs/superpowers/plans/2026-09-05-s3-acl-owner.md`)

Incremental surface + authn/authz work implemented on top of the original plan (FR-033/SC-009, added to `spec.md` + `contracts/s3-surface.md`; executed as the sdd plan's Tasks 1-16); the cucumber scenarios each entry references carry the new `@FR-033` (and `@SC-009`) scenario tags — the features host pre-existing keep-legs with older tags, so the tags are per-scenario, not feature-level. `STATE_VERSION` stays 1 — the owner/ACL row elements are additive (user ruling 2026-09-05). The authorization matrix's one-home is the `tinio-auth` crate's unit-pinned truth table.

- [X] T127 [US1/US3] tinio-core `acl` module (`crates/tinio-core/src/acl.rs` + `percent.rs`): `OwnerId` (64 lowercase hex OR exactly the anonymous special canonical ID — Task 7 controller ruling; a different 32-hex value stays rejected, and the config layer rejects an owner/user colliding with the anonymous ID), `Permission` (the five AWS wire strings), `GroupUri` + the three documented group-URI constants (no EC2 constant — grilling Q5), `Grantee`/`Grant`/`Acl`/`AclGrants` (canonical sorted grants wire, ≤100 cap, dedupe), `Acl::default_private`/`to_grants_wire`/`from_grants_wire` (self-heal to `default_private(None)`), the built-in defaults (`default_owner_id` = `hex(SHA-256("tinio"))`, `DEFAULT_OWNER_DISPLAY_NAME`), `AclError`; the tags codec's percent layer extracted to `percent.rs` (byte format unchanged) — FR-033 (unit tests first)
- [X] T128 [US1/US3] tinio-store shared-row extension: `OBJECT_META` 8-tuple / `BUCKETS` 4-tuple / `UPLOADS` 5-tuple (owner_wire + acl_wire appended; in `tinio-store/src/{meta,bucket,upload}.rs` + `acl.rs` decode helpers, self-healing like tags); the schema-assertion test extended as **compile-time shape pins via round-trip contract tests** — redb 4.2.0 exposes no `value_arity()`/`key_arity()` getter (verified in the vendored source), the controller-sanctioned fallback: each pin writes + reads back a full row through the public handle, destructuring the exact new tuple arity (a stray element fails the equality assertion); `STATE_VERSION` stays 1 — FR-033
- [X] T129 [US1/US3] Storage contract: `BucketOps` gains `get_bucket_acl`/`put_bucket_acl` (replace-all, the owner element preserved by the store) and `create_bucket` gains `owner: Option<&OwnerId>` + `acl: &Acl` (None = the empty owner wire — the lazy default owner, review B4); `list_buckets` gains `owner: Option<&OwnerId>` (walk-time, pre-pagination filter — review P2#4) and the B4-amendment third param `lazy_default: Option<&OwnerId>` (the effective-owner compare, uniform with `row_owner`; `None` = the strict compare); `ObjectOps` gains the object ACL pair + owner/ACL on `commit_object`/`copy_object`, `MultipartOps::create_multipart_upload` gains both (applied by the completion internally — never re-ferried), `Info`/`MultipartUpload` gain `owner`+`acl` — FR-033
- [X] T130 [US1/US3] tinio-fs: the ACL stores over the shared rows (tags-pattern read-modify-write, preserve the rest of the row), the unix ownership/mode hardening (`File::create` → 0600, dirs → 0700, post-write chown to the mapped owner/bucket-owner uid via `FsOptions.owner_uids`; `rewrite_preserving` keeps the owner/ACL row; chown-failure after rename lands → fail-closed removal; implicit prefix dirs chowned to the bucket owner; unmapped owners → no chown) — FR-033 (unix test runs = the WSL2 gate)
- [X] T131 [US1/US3] tinio-mem: the same stores over the in-memory redb, conformance green — FR-033
- [X] T132 [US1/US3] tinio-util conformance harness: ACL storage round-trip + owner preservation + default-private, owner/ACL propagation across copy/rename/multipart, `Info` owner on head/listing, `list_buckets` filter-then-paginate with the continuation-token math applied to the filtered set, the interleaved A/B owner-walk legs (review P2#4) — FR-033
- [X] T133 [US1/US3] tinio-auth crate: `identity.rs` (User/Identity, the root-from-`[auth]`+`[owner]`, the three-way owner resolution), `auth.rs` (`ConfigAuth`, the FR-008-phrasing unknown-key error), `access.rs` (`AclAccess` — the pipeline: principal → `expected_bucket_owner` → op mapping → owner bypass → grant evaluation → fail-closed; the deny-first three tiers; the B1 existence head; the B2 coarse DeleteObjects gate), `matrix.rs` (the unit-pinned truth table), `canned.rs` (the eight canned expansions, the two-owner signature — review A6, `aws-exec-read` accepted-but-private), `error.rs` — FR-033/FR-008
- [X] T134 [US1/US3] tinio-server feature gate + the four ACL ops: the `acl` cargo feature (default on), `identity` field + `owner_for`/`row_owner`/`acl_owner` helpers, `get/put_bucket_acl`/`get/put_object_acl` (existence-head get, replace-all put with the fail-closed body-Owner match → 400, strict request-level grant validation vs the codec's read-path self-heal, the Content-MD5 three-state gate — presence + well-formedness only, the digest-equality deviation, review A7) — FR-033
- [X] T135 [US1/US3] Write-path ACL headers + owner recording: `op_put_object`/`op_copy_object`/`op_create_bucket`/`op_create_multipart_upload` expand canned/`x-amz-grant-*` into the row ACL (canned+grants → 400, email grantee → 400), record the B4 owner (signed user's canonical ID / the anonymous special ID / the empty wire under no-identity), overwrite replaces the owner element + ACL, copy never inherits, PostObject rides the delegated put path with the handler-side B1 destination head (the access layer applies the base bucket-WRITE only — recorded deviation: the form key is invisible pre-deserialization) — FR-033
- [X] T136 [US1/US3] Owner surface + requester filtering: list v1/v2 entry `Owner`, `list_buckets` `Owner` + requester-filtered walk (enforced mode), `expected_bucket_owner` at the access check, DeleteObjects per-key authorization in the handler (per-key `AccessDenied` Error entries in the 200) — FR-033
- [X] T137 [US1/US3] Server wiring: `DataPlane::new_with_acl` (ConfigAuth + AclAccess composed with MetricS3, active only when feature on AND `caps.acl` on), feature-off/toggle-off keep the legacy root-only path with the accept-and-drop posture, `serve.rs` identity assembly + the owner-uid map into `FsOptions`, the module-doc 403-ambiguity note (ACL denial vs OS `PermissionDenied` vs symlink refusal — the mapping logs the source at debug) and the `/metrics`-outside-ACL note — FR-033
- [X] T138 [US1/US3] tinio-config: `[s3] acl` capability (default on, `@minimal-caps` clears it), `[owner]`/`[[users]]` schemas + garde (unique access keys, unique canonical IDs incl. the root-collision rejection, the anonymous-ID rejection at parse), `local_uid` unix-only (Windows garde rejection), the hard startup chown-privilege validation in `serve.rs` (real chown probe, fail-closed config error) — FR-033/FR-021
- [X] T139 [US1/US3] Server unit suite (MemoryStorage): the four ops round-trip/replace/404 + Content-MD5 + body-Owner mismatch, the cap gate, write-path headers (canned/explicit/conflict/email), owner recording incl. overwrite-replaces-owner, `list_buckets` filtering, DeleteObjects per-key denial, toggle-off accept-and-drop — FR-033
- [X] T140 [US1/US3] Cucumber `acl.feature` + the SigV4 signer: a hand-rolled minimal SigV4 signer over `hmac` + `sha2` dev-deps (the standard 4-step HMAC chain; golden-vector unit tests ride the traceability binary) driving the 8 signed/anonymous scenarios (public-read + anonymous GET, private → anonymous 403, READ-grant signed GET, `expected_bucket_owner` mismatch, overwrite owner rule, `list_buckets` filtering, put-ACL `Content-MD5`, `@acl-off` 501) — FR-033/SC-009 (`@acl` spawn branch + `@acl-off` legacy plane in the steps harness)
- [X] T141 [US1/US3] Benches + the boto3 journey: `tinio-server/benches/listing.rs`/`multipart_assembly.rs` and `tinio-fs/benches/streaming.rs` adapted to the Tasks 3-9 contract shapes (create_bucket 3-arg, CMU 6-arg); `tests/boto3_journey.py` gains the ACL legs (create-bucket, `cp --acl public-read`, anonymous GET — stdlib urllib, the venv carries no `requests` — `get-bucket-acl` echo, `--expected-bucket-owner` mismatch 403, `list_buckets` per principal incl. the anonymous 403) — FR-033 (boto3 run = the user's boto3 leg)
- [X] T142 [US1/US3] Specs & docs: FR-033/SC-009 written into `spec.md` (incl. the §5a payload-modes + ownership-mapping contract and the edge-case entries) and `contracts/s3-surface.md`, `[owner]`/`[[users]]` + `local_uid` + the OS-mapping validation into `contracts/config.md`, the row shapes into `meta-redb-spec.md` §5.2 + `data-model.md`, task entries here, checklist items in `checklists/compatibility.md` (CHK052-055) and `checklists/security.md` (CHK042-045), the feature scenarios tagged with the new IDs — FR-033/SC-009
