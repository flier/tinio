...
- [X] T047 [P] [US1] Implement the S3 buckets group in `crates/tinio-server/src/backend/buckets.rs`: CreateBucket/DeleteBucket/HeadBucket/ListBuckets/GetBucketLocation (CreationDate from buckets.json, GetBucketLocation → `us-east-1`); storage errors → S3 error codes (NoSuchBucket, BucketAlreadyExists, BucketNotEmpty, InvalidBucketName); unit tests written first
