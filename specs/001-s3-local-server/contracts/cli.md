...
- `.tinio/` integrity: stale state file (running server or crashed), stale unix socket (probed), orphaned meta-store entries, orphaned `buckets.json` entries, abandoned multipart uploads, stale temp files (state/socket checks apply when built with the `api` feature);
